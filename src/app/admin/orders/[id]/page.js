...
function OrderDetail({ params }) {
    ...
    return (
        <>
            <div className="left">
                <h2 className="order-detail-title">Chi tiết đơn hàng <span style={{ color: 'red' }}>#{order._id}</span></h2>
            </div>
            ...
            <div className="table-data">
                            ...
                            {order.cart && order.cart.map((product, index) => (
                                <tr key={product._id} className="order-table-row">
                                    ...
                                    <td className="order-table-cell">
                                        <img src={`${process.env.NEXT_PUBLIC_IMAGE_BASE_URL}/${product.image}`} alt={product.name} className="order-product-image" />
                                    </td>
                                    ...
                                    <td className="order-table-cell">{(product.price * product.quantity).toLocaleString('vi-VN')}đ</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div className="order-detail">
                    ...
                    <p className="order-detail-info">Ngày đặt hàng: {new Date(order.createdAt).toLocaleDateString('vi-VN')}</p>
                    <p className="order-detail-info">Thời gian đặt hàng: {new Date(order.createdAt).toLocaleTimeString('vi-VN')}</p>
                    <p className="order-detail-info">Tạm tính: {calculateTotalAmount(order.cart).toLocaleString('vi-VN')}đ</p>
                    <p className="order-detail-info">Phí vận chuyển: 38.000đ</p>
                    <p className="order-total">Tổng cộng: <span style={{ color: 'red' }}>{(calculateTotalAmount(order.cart) + 38000).toLocaleString('vi-VN')}đ</span></p>
                    ...
                    <div className='order-detail-body'>
                        ...
                        <p className="order-detail-info">
                            Phương thức thanh toán: {order.paymentMethod && order.paymentMethod === "1" ? "Thanh toán khi nhận hàng" : "Thanh toán Momo"}
                        </p>
                        <div className='flex' style={{ display: 'flex' }}>
                            <p className="order-detail-info">Trạng thái đơn hàng:</p>
                            <form onSubmit={handleStatusUpdate} className="order-status-form" style={{ marginTop: -8, marginLeft: 12 }}>
                                ...
                                <button type="submit" className="update-status-btn">Cập nhật</button>
                            </form>
                        </div>
                    </div>
                </div>
